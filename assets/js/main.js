(() => {
  ...
  const carouselBlock = document.querySelector("[data-carousel]");
  if (carouselBlock) {
    const track = carouselBlock.querySelector(".carousel__track");
    const items = Array.from(track.querySelectorAll("[data-carousel-item]"));
    const prevBtn = carouselBlock.querySelector(".carousel__nav--prev");
    const nextBtn = carouselBlock.querySelector(".carousel__nav--next");
    ...
    const startAutoPlay = () => {
      ...
      autoPlayTimer = window.setInterval(playNext, 5000);
    };
    ...
    visibilityObserver.observe(carouselBlock);
